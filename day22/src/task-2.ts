...
import { type Brick, parseInput } from "./parse-input.js";
...
const originalSpace = fall(input);

let result = 0;
for (let brick = 0; brick < input.length; ++brick) {
  const nextBrick = [...input.slice(0, brick), ...input.slice(brick + 1)];

  const nextSpace = fall(nextBrick);
  ...
  for (let z = 1; z <= maxZ; ++z) {
    for (let x = 0; x < maxX; ++x) {
      for (let y = 0; y < maxY; ++y) {
        const originalBlock = originalSpace[z][x][y];
        if (originalBlock === undefined) {
          continue;
        }
        let nextBlock = nextSpace[z][x][y];
        if (nextBlock !== undefined) {
          // Because we remove one block
          if (nextBlock >= brick) {
            nextBlock++;
          }
        }
        if (originalBlock !== nextBlock) {
          diff.add(originalBlock);
        }
      }
    }
  }
